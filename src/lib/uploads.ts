...
export const mergeBackupImages = (
  ...
  const uniqueEntries = Array.from(new Map(entries.map((entry) => [entry.fileName, entry] as const)).values());

  for (const { fileName, buffer } of uniqueEntries) {
    const ext = path.extname(fileName).toLowerCase();
    const base = sanitizeBase(path.basename(fileName, ext)) || "image";
    let targetName = fileName;
    let fullPath = path.join(uploadsDir, targetName);

    while (fs.existsSync(fullPath)) {
      targetName = `${base}-${randomUUID()}${ext}`;
      fullPath = path.join(uploadsDir, targetName);
    }

    fs.writeFileSync(fullPath, buffer);
    nameMap[fileName] = targetName;
  }
...
export const clearAllImages = (): void => {
  for (const fileName of listImageNames()) {
