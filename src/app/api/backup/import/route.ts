...
import { mergeListings, replaceAllListings, type ListingRestoreInput } from "@/lib/db";
import { isSafeImageName, mergeBackupImages, replaceAllImages } from "@/lib/uploads";
...
export async function POST(request: Request) {
  try {
    ...
    if (mode === "replace") {
      ...
      const imageNameMap = mergeBackupImages(imageEntries);
      const mappedListings = listings.map((entry) => ({
        ...entry,
        imageFileNames: entry.imageFileNames.map((fileName) => imageNameMap[fileName] ?? fileName),
      }));
      mergeListings(mappedListings);
    }

    return NextResponse.json({
      ok: true,
      mode,
      importedListings: listings.length,
      importedImages: imageEntries.length,
    });
  } catch (error) {
